...
keyFileName = fs.readdirSync("./keystore/").filter(fileName => fileName.endsWith(fromAddress.substring(2)))[0];
encrypted_key = fs.readFileSync("./keystore/" + keyFileName, 'utf-8');
fromAccount = web3.eth.accounts.decrypt(encrypted_key, 'password')
...
async function sendTransaction() {
    nonce++;
    // signedtx = await fromAccount.signTransaction({ "data": tx, "gas": 4000000, "nonce": nonce });
    // console.log("signedTx", signedtx);

    var privateKey = Buffer.from('64331085d24752ae9a9004f4b01cb9421cb3cc5756398536b3a3ca11a95e6814', 'hex')
...
async function loop() {
    ...
    switch (contractType) {
        case "KVStore":
            ...
            tx = contract.methods.set("hello", "world").encodeABI();
            console.log("tx", tx)
            break;
        case "StandardContract":
