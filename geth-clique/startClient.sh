...
if [ $test = "latency" ]; then
    ...
    nohup node ./latency.js $endpoint $account $wl $adjTxLimit $startTime $clientId $minerCount $clientCount $test "geth-clique" $timeStamp > $LOG_DIR/$test"_client_"$clientId"_"$wl"_"$minerCount"_miners_"$clientCount"_clients_"$txRate"_txRate_"$txLimit"_txLimit_"$timeStamp 2>&1 &
else
    ...
    nohup node ./tpsUnsigned.js $endpoint $account $wl $adjTxRate $adjTxLimit $startTime $clientId $minerCount $clientCount $test "geth-clique" $timeStamp > $LOG_DIR/$test"_client_"$clientId"_"$wl"_"$minerCount"_miners_"$clientCount"_clients_"$txRate"_txRate_"$txLimit"_txLimit_"$timeStamp 2>&1 &
fi